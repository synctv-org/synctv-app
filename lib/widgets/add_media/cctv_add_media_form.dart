...
class _CctvAddMediaFormState extends State<CctvAddMediaForm> {
  ...
  Future<void> _submit() async {
    ...
    if (parsed == null) {
      MessageUtils.showError(context, 'Invalid CCTV URL or video ID');
      return;
    }
    setState(() => _loading = true);
    try {
      ...
      if (widget.onSubmit case final submit?) {
        ...
        final resolved =
            _resolved ?? await SyncTvService.resolveCctv(request.resource);
        final source = resolved.sourceConfig;
        await SyncTvService.addCctvMedia(
          widget.roomId,
      ...
      MessageUtils.showSuccess(context, 'CCTV source added');
      setState(() {});
    } catch (error) {
      if (mounted) {
