    ...
    try {
      _resolved =
          await (widget.onResolve?.call(_request.resource) ??
              SyncTvService.resolveHuya(_request.resource));
    } catch (error) {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (widget.onSubmit case final submit?) {
        ...
        final resolved =
            _resolved ?? await SyncTvService.resolveHuya(request.resource);
        ...
        await SyncTvService.addHuyaMedia(
          widget.roomId,
      ...
      MessageUtils.showSuccess(context, 'Huya source added');
      setState(() {});
    } catch (error) {
      if (mounted) MessageUtils.showError(context, '$error');
    } finally {
