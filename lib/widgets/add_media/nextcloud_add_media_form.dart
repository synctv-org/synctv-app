...
class _NextcloudAddMediaFormState extends State<NextcloudAddMediaForm> {
  ...
  void _search() {
    if (_searchController.text.trim().length < 3) {
      MessageUtils.showError(context, 'Enter at least 3 characters');
      return;
    }
    setState(() => _page = 1);
    _load();
  ...
  Future<NextcloudFileListPage> _defaultLoader(
    ...
    return switch (mode) {
      NextcloudBrowseMode.folder => SyncTvService.listNextcloudFiles(
        bind.serverId,
        path,
        page: page,
        pageSize: pageSize,
      ...
      NextcloudBrowseMode.favorites => SyncTvService.listNextcloudFavorites(
        bind.serverId,
        page: page,
        pageSize: pageSize,
        instanceName: bind.providerInstanceName,
      ),
      NextcloudBrowseMode.search => SyncTvService.listNextcloudFiles(
        bind.serverId,
        path,
        page: page,
        pageSize: pageSize,
  ...
  Future<void> _addMedia(NextcloudFileItemInfo item) => _runAdd(() async {
    final bind = _bind!;
    await SyncTvService.addNextcloudMedia(
      widget.roomId,
