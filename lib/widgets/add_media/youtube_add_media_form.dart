...
import 'package:synctv_app/models/youtube_source_config.dart';
...
import 'package:synctv_app/widgets/add_media/youtube_playlist_preview.dart';
...
class _YoutubeAddMediaFormState extends State<YoutubeAddMediaForm> {
  ...
  Future<void> _loadPreview({bool loadMore = false}) async {
    ...
    if (normalized == null) {
      MessageUtils.showError(context, 'Invalid YouTube source');
      return;
    }
    ...
    try {
      if (_mode == YoutubeAddMode.video) {
        ...
        _resolved = resolve != null
            ...
            : await SyncTvService.resolveYoutube(
                normalized,
                instanceName: _instanceName,
              );
      } else {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      MessageUtils.showSuccess(context, 'YouTube source added');
      setState(() {});
    } catch (error) {
      if (mounted) MessageUtils.showError(context, '$error');
    } finally {
  ...
  Future<void> _addSelectedPreviewItems(
    ...
    try {
      ...
      _playlistPreview = null;
      widget.onDraftChanged(false);
      MessageUtils.showSuccess(context, 'YouTube media added');
      setState(() {});
    } catch (error) {
      if (mounted) MessageUtils.showError(context, '$error');
    } finally {
