    ...
    try {
      _resolved =
          await (widget.onResolve?.call(_request.resource) ??
              SyncTvService.resolveDouyu(_request.resource));
    } catch (error) {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (widget.onSubmit case final submit?) {
        ...
        final resolved =
            _resolved ?? await SyncTvService.resolveDouyu(request.resource);
        await SyncTvService.addDouyuMedia(
          widget.roomId,
      ...
      MessageUtils.showSuccess(context, 'Douyu source added');
      setState(() {});
    } catch (error) {
      if (mounted) MessageUtils.showError(context, '$error');
    } finally {
