...
class _SeafileAddMediaFormState extends State<SeafileAddMediaForm> {
  ...
  void _search() {
    if (_repositoryId.isEmpty) {
      MessageUtils.showError(context, 'Select a library first');
      return;
    }
  ...
  Future<SeafileFileListPage> _defaultLoader(
    ...
    if (mode == SeafileBrowseMode.starred) {
      return SyncTvService.listSeafileStarred(
        bind.serverId,
        page: page,
        pageSize: pageSize,
    ...
    if (repositoryId.isEmpty) {
      return SyncTvService.listSeafileRepositories(
        bind.serverId,
        page: page,
        pageSize: pageSize,
        instanceName: bind.providerInstanceName,
    ...
    return SyncTvService.listSeafileFiles(
      bind.serverId,
      repositoryId,
      path,
      page: page,
  ...
  Future<void> _addMedia(SeafileFileItemInfo item) => _runAdd(() async {
    final bind = _bind!;
    await SyncTvService.addSeafileMedia(
      widget.roomId,
