...
class DLNADevice {
  ...
  static const Set<String> _renderingControlActions = {
    'SetMute',
    'GetMute',
    'SetVolume',
    'GetVolume',
  ...
  DLNADevice(this.info) {
    positionPoller = PositionPoller(this, currPosition);
  }
  ...
  Future<String> request(String action, List<int> data) {
    final soapAction = _renderingControlActions.contains(action)
        ? 'RenderingControl'
        : 'AVTransport';
...
class PositionPoller {
  ...
  void _fetchPositionPeriodically() async {
    ...
    try {
      ...
      debugPrint('$e. Will try again after 2 seconds.');
    } finally {
...
class _UpnpMessageParser {
  final String message;
  _UpnpMessageParser(this.message);
...
class DLNAManager {
  static const String upnpIpv4 = '239.255.255.250';
  static const int upnpPort = 1900;
  final InternetAddress upnpAddressIpv4 = InternetAddress(upnpIpv4);
  ...
  Future<DeviceManager> start({reusePort = false}) async {
    ...
    if (Platform.isIOS) {
      ...
      for (final interface in interfaces) {
        final value = Uint8List.fromList(
          upnpAddressIpv4.rawAddress + interface.addresses[0].rawAddress,
        );
      ...
      _socketServer!.joinMulticast(upnpAddressIpv4);
    }
    final socketClient = await RawDatagramSocket.bind(
      InternetAddress.anyIPv4,
      0,
    );

    _clientSubscription = socketClient.listen((RawSocketEvent event) {
      if (event == RawSocketEvent.read) {
        while (true) {
          final datagram = socketClient.receive();
    ...
    _serverSubscription = _socketServer?.listen((RawSocketEvent event) {
      if (event == RawSocketEvent.read) {
        while (true) {
          final datagram = _socketServer?.receive();
          if (datagram == null) break;
    ...
    _sendSearchRequest(socketClient);
    _sender = Timer.periodic(const Duration(seconds: 2), (Timer t) async {
      _sendSearchRequest(socketClient);
    });
    return dm;
  }
  ...
  Future<void> _sendSearchRequest(RawDatagramSocket socket) async {
    ...
    for (int i = 0; i < stList.length; i++) {
      ...
      final msg = 'M-SEARCH * HTTP/1.1\r\n'
          ...
          'MAN: "ssdp:discover"\r\n\r\n';
      socket.send(msg.codeUnits, upnpAddressIpv4, upnpPort);
