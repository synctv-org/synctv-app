...
class BilibiliGeetestService {
  ...
  static Future<BilibiliGeetestResult> _verifyWithDesktopWindow({
    ...
    final webview = await WebviewWindow.create(
      configuration: CreateConfiguration(
        ...
        userDataFolderWindows:
            '${appDir.path}${Platform.pathSeparator}geetest_webview',
      ),
    );
    final completer = Completer<BilibiliGeetestResult>();
    Timer? timer;
    var closing = false;
    ...
    void completeMessage(String message) {
      ...
      try {
        completer.complete(parseBilibiliGeetestMessage(message));
      } catch (error) {
        completeError(error);
      }
    }
    ...
    webview.onClose.then((_) {
      if (!completer.isCompleted) {
        completeError(StateError('Bilibili 安全验证已取消'));
      }
    });
    ...
    if (Platform.isLinux || Platform.isMacOS) {
      webview.registerJavaScriptMessageHandler(
        _bridgeName,
        (_, body) => completeMessage(_normalizeDesktopMessageBody(body)),
      );
    }
    ...
    timer = Timer(timeout, () {
      completeError(TimeoutException('Bilibili 验证超时', timeout));
    });

    webview.launch(
      Uri.dataFromString(
        buildBilibiliGeetestHtml(gt: gt, challenge: challenge),
        mimeType: 'text/html',
        encoding: utf8,
      ).toString(),
    );
  ...
  static String _normalizeDesktopMessageBody(Object? body) {
  ...
  static bool get supportsEmbeddedWebView {
    return _supportsDialogWebView || _supportsDesktopWebView;
  }
}
