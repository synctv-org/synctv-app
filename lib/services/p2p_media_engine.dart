...
class P2pPieceRequestCancellation {
...
class P2pMediaEngine {
  ...
  Future<Uri> localize({
    ...
    final resource = _registerResource(
      ...
      logicalKey: 'root',
      shareable: true,
      manifestKind: _manifestKind(format, upstream),
      isDirectory: false,
    );
  ...
  Future<_OpenedOriginOrPeer?> _openOriginWithPeerRetry(
    ...
    Future<void> observeOrigin() async {
      ...
      if (origin != null && !result.isCompleted) {
    ...
    final timeout = Timer(originHeaderTimeout, () {
  ...
  Future<int> _resolveResourceLength(_GatewayResource resource) async {
    ...
    if (metadata != null && metadata.length >= 8) {
      final length = ByteData.sublistView(metadata).getUint64(0);
      if (length > 0) {
        resource.originAcceptsRanges = _decodeRangeCapability(metadata);
  ...
  Future<void> _rememberResourceLength(
    ...
    if (cached != null &&
        cached.length >= 9 &&
        ByteData.sublistView(cached).getUint64(0) == length &&
        cached[8] == rangeCapability) {
      return;
    }
    final metadata = Uint8List(9);
    ByteData.sublistView(metadata).setUint64(0, length);
    metadata[8] = rangeCapability;
  ...
  String _rewriteHlsManifest(String manifest, _GatewayResource parent) {
    ...
    for (final line in lines) {
      ...
      if (line.startsWith('#')) {
        ...
        rewritten.add(
          line.replaceAllMapped(RegExp(r'URI="([^"]+)"'), (match) {
            ...
            final logicalKey = isKey
                ? '${parent.logicalKey}:key:${resourceIndex++}'
                : '${parent.logicalKey}:resource:$discontinuitySequence:'
                      '$mediaSequence:${line.split(':').first}:${resourceIndex++}';
            final child = _registerResource(
              upstream: parent.upstream.resolve(raw),
              ...
              manifestKind:
                  !isKey &&
                      (attributeUriIsPlaylist ||
                          _looksLikeHlsManifest(parent.upstream.resolve(raw)))
                  ? _P2pManifestKind.hls
                  : _P2pManifestKind.progressive,
              isDirectory: false,
            );
            return 'URI="${_localUri(child)}"';
          }),
        );
        continue;
      }
      final isManifest =
          nextUriIsPlaylist ||
          _looksLikeHlsManifest(parent.upstream.resolve(line));
      nextUriIsPlaylist = false;
      final logicalKey = isManifest
          ? '${parent.logicalKey}:manifest:${resourceIndex++}'
          : '${parent.logicalKey}:segment:$discontinuitySequence:'
                '${mediaSequence + segmentIndex++}';
      final child = _registerResource(
        upstream: parent.upstream.resolve(line),
        headers: parent.headers,
        swarmId: parent.swarmId,
        logicalKey: logicalKey,
      ...
      rewritten.add(_localUri(child).toString());
    }
  ...
  String _rewriteDashManifest(String manifest, _GatewayResource parent) {
    ...
    void rewriteElement(XmlElement element, Uri inheritedBase) {
      for (final attribute in element.attributes) {
        ...
        attribute.value = _registerDashReference(
          parent: parent,
          upstream: inheritedBase.resolve(raw),
          logicalKey: '${parent.logicalKey}:dash-resource:${resourceIndex++}',
          kind: kind,
        );
      }
      ...
      if (element.name.local == 'Location') {
        ...
        if (raw.isNotEmpty) {
          final local = _registerDashReference(
            parent: parent,
            upstream: inheritedBase.resolve(raw),
            logicalKey: '${parent.logicalKey}:dash-resource:${resourceIndex++}',
      ...
      for (var index = 0; index < baseUrls.length; index++) {
        ...
        final upstream = inheritedBase.resolve(raw);
        if (index == 0) descendantBase = upstream;
        if (!identical(element, root) && _isSafeRelativeDashReference(raw)) {
          continue;
        }
        final logicalKey = '${parent.logicalKey}:dash-base:${baseIndex++}';
        final local = raw.endsWith('/') || upstream.path.endsWith('/')
            ? _localDirectoryUri(
                _registerDashDirectory(parent, upstream, logicalKey),
              )
            : _localUri(
                _registerResource(
                  upstream: upstream,
    ...
    rewriteElement(root, parent.upstream);
    if (rootBaseUrls.isEmpty) {
      final directory = _registerDashDirectory(
        parent,
        parent.upstream.resolve('.'),
        '${parent.logicalKey}:dash-root',
      );
      root.children.insert(
        0,
        XmlElement(XmlName.parts('BaseURL'), const [], [
          XmlText(_localDirectoryUri(directory).toString()),
        ]),
      );
    }
  ...
  String _registerDashReference({
    required _GatewayResource parent,
    required Uri upstream,
    required String logicalKey,
    required _DashReferenceKind kind,
  }) {
    if (kind == _DashReferenceKind.media &&
        upstream.toString().contains(r'$')) {
      ...
      final scope = upstream.replace(
        path: path.substring(0, suffixStart),
        query: null,
        fragment: null,
      );
      final directory = _registerDashDirectory(parent, scope, logicalKey);
      final suffix = StringBuffer(path.substring(suffixStart));
      if (upstream.hasQuery) suffix.write('?${upstream.query}');
      if (upstream.hasFragment) suffix.write('#${upstream.fragment}');
      return '${_localDirectoryUri(directory)}$suffix';
    }

    if (upstream.path.endsWith('/')) {
      return _localDirectoryUri(
        _registerDashDirectory(parent, upstream, logicalKey),
      ).toString();
    }
    final resource = _registerResource(
      ...
      manifestKind: kind == _DashReferenceKind.manifest
          ? _P2pManifestKind.dash
          : _P2pManifestKind.progressive,
      isDirectory: false,
    );
    final local = _localUri(resource);
    return upstream.hasFragment
        ? local.replace(fragment: upstream.fragment).toString()
        : local.toString();
  }
  ...
  _GatewayResource _registerDashDirectory(
    _GatewayResource parent,
    Uri upstream,
    String logicalKey,
  ) {
    final directoryUri = upstream.path.endsWith('/')
        ? upstream
        : upstream.resolve('.');
    return _registerResource(
      upstream: directoryUri,
      headers: parent.headers,
      swarmId: parent.swarmId,
      logicalKey: logicalKey,
      shareable: true,
  ...
  static bool _looksLikeHlsManifest(Uri uri) {
    return _mediaPath(uri).endsWith('.m3u8');
  }

  static String _mediaPath(Uri uri) {
    ...
    for (final value in uri.queryParameters.values) {
      ...
      if (nestedPath != null && _knownMediaPath(nestedPath)) return nestedPath;
    }
    return path;
  }

  static bool _knownMediaPath(String path) =>
      path.endsWith('.m3u8') ||
      path.endsWith('.ts') ||
      path.endsWith('.m4s') ||
      path.endsWith('.mp4') ||
  ...
  static bool? _decodeRangeCapability(Uint8List metadata) {
